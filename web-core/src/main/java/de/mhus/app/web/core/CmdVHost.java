...
public class CmdVHost extends AbstractCmd {
    ...
    @Argument(
            ...
            required = true,
            description =
                    "Command: list, info, config, use, release, current, restart, headerfactories",
            multiValued = false)
    String cmd;
    ...
    public Object execute2() throws Exception {
        ...
        if (cmd.equals("info")) {
            ...
            for (Entry<String, VirtualHost> entry :
                    CherryApiImpl.instance().getVirtualHosts().entrySet()) {
                VirtualHost vhost = entry.getValue();
                Bundle bundle = vhost.getBundle();
                out.addRowValues(
                        entry.getKey(),
                        vhost.getName(),
                        vhost.getClass().getName(),
        ...
        if (cmd.equals("use")) {
            CherryCallContext callContext =
                    new CherryCallContext(null, null, new CherryResponseWrapper(null), vhost);
