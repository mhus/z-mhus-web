...
public class AccessFilter extends MLog implements WebFilter {
	...
	public boolean doFilterBegin(UUID instance, InternalCallContext call) throws MException {
		...
		if (isStaticContentPath(path)) {
            ...
            if (call.getVirtualHost().prepareHead(call, "_dynamic_content", false) == null) {
    			call.getHttpResponse().setHeader("Cache-Control","no-cache, no-store, must-revalidate");
        ...
        log().d("access",userName,call.getVirtualHost().getName(),call.getRemoteIp(), call.getHttpRequest().getRemoteAddr(),call.getHttpHost(),call.getHttpMethod(),call.getHttpPath());

		return true;
	}

	/**
	 * Overwrite to grant access. The default is using access_groups and access_users from file configuration.
	 * 
	 * @param call
	 * @param user
	 * @param config
	 * @return
	 */
	protected boolean hasAccess(InternalCallContext call, UserInformation user, IReadProperties config) {
	    ...
	    if (users != null) {
	        for (String u : users.split(","))
	            if (user.getUserName().equals(u)) return true;
	    }
	    return false;
    }
