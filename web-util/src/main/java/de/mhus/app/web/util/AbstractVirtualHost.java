...
public abstract class AbstractVirtualHost extends MLog implements VirtualHost {
     ...
     *
     * <p>See https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html
     *
     * @param call
     */
    @Override
    public void doRequest(InternalCallContext call) {
        try {
            ...
            // execute filters
            if (!doFiltersBegin(call)) return;

            if (doActiveAreas(call)) return;
    ...
    protected void setConfigAliases(String[] aliases) {
        ...
        if (externalAliases != null) MCollection.addAll(a, externalAliases);
        // configured aliases
        if (aliases != null) MCollection.addAll(a, aliases);
        // set
    ...
    public boolean doActiveAreas(InternalCallContext call) throws MException {
        ...
        for (ActiveAreaContainer area : areas)
            if (path.startsWith(area.alias)) {
                if (area.area.doRequest(instanceId, call)) return true;
            }
        return false;
    }
    ...
    /**
     * combine the name of the config file or section with the current profile name. If profile is
     * not set the name itself will be returned.
     *
